...
<div
	class={device.last_state.status < 0
		? 'personal_computer square inactive'
		: 'personal_computer square active'}
>
	<div class="icon center noselect">
		<img
			...
			alt="PC on"
			class="stat_img"
			class:hidden={device.last_state.status != 1}
		/>
		<img
			src="./img/desk_off.svg"
			alt="PC off"
			class="stat_img"
			class:hidden={device.last_state.status != 0}
		/>
		<img
			src="./img/desk_wait.svg"
			alt="PC waiting"
			class="stat_img"
			class:hidden={device.last_state.status != -1}
		/>
		<img
			...
			alt="PC disconnected"
			class="stat_img"
			class:hidden={device.last_state.status != -2}
		/>
		<span class="title">{device.name}</span>
	</div>

	<button class="btn_main" on:click={changeDeviceStatus(device)} />
	{#if device.last_state.status != -1}
		<button
