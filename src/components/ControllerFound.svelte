<script>
    ...
    function getControllerInfo() {
        fetch(`http://${controller.ipv4_interface}/info`, {referrerPolicy: "unsafe-url", credentials: "omit", mode: "no-cors"})
            .then( (res) => res.json() )
            .then( (data) => {
...
<div class:unavailable={unavailable}>
    <a href={`http://${controller.ipv4_interface}/`} class="btn_grey" target="_blank">{controller._id}</a>
</div>
